...
```{r}
svm_housing = svm(MEDV ~ ., data=housing_train, kernel="radial", gamma=0.08)
...
```{r}
grid_gbm = expand.grid(.interaction.depth = (1:5)*2, .n.trees = (1:10)*25, .shrinkage = .1, .n.minobsinnode = .1)

bootControl = trainControl(number = 200)
gmb_housing = train(housing_train[,1:13], housing_train[,14], method="gbm", trControl = bootControl, verbose=F, bag.fraction = 1, tuneGrid = grid_gbm)
```

```{r}
set.seed(7)
boosting_housing = gbm(MEDV ~ ., data = housing_train, n.trees = 200, interaction.depth = 6, shrinkage = 0.1, n.minobsinnode = 0.1)

pred_boosting = predict(boosting_housing, housing_test, n.trees = 200, interaction.depth = 6, shrinkage = 0.1, n.minobsinnode = 0.1)
...
m = ncol(housing_train)/3
set.seed(6)
rf_housing = randomForest(MEDV ~ ., housing_train, ntree=100, mtry=m)
prob_rf = predict(rf_housing,housing_test[,-which(names(housing_test) == "MEDV")])
mse(housing_test$MEDV, prob_rf)
print(rf_housing)
plot(rf_housing)
```

```{r}
head(housing_test$MEDV)
head(prob_rf)
```
